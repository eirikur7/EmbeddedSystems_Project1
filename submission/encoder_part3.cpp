...
void Encoder::init()
{
    ...
    // Setting ISC01 as well would trigger on one edge only and halve the resolution.
    EICRA |= (1 << ISC00);
    EICRA &= ~(1 << ISC01);

    EIMSK |= (1 << INT0); // enable INT0
    sei();                // enable interrupts, main() skips the Arduino init()
}

// Runs on every edge of C1.
// The hardware already found the edge, so there is no last state to compare against.
// The level of C2 right now gives the direction.
